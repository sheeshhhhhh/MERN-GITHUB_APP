...
import { PROGRAMMING_LANGUANGES } from "../utils/constant";

import toast from 'react-hot-toast'

const Repo = ({repo}) => {

	const releasedon = formatDate(repo.created_at)

	const handleCloneclick = async (repo) => {
		try {
			await navigator.clipboard.writeText(repo.clone_url)
			toast.success("Clone copied")
		} catch (error) {
	...
	return (
		<li className='mb-10 ms-7'>
			...
			{PROGRAMMING_LANGUANGES[repo.language] ? (
				<img src={PROGRAMMING_LANGUANGES[repo.language]} alt="Programming language icon" className="h-8" />
			): null}
		</li>
	);
};
